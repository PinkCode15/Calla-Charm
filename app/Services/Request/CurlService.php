...
class CurlService
{
    ...
    public function __construct($options = array())
    {
        if (!function_exists('curl_init')) {
            throw new CallaCharmCurlException('cURL module must be enabled!');
        }
     ...
     * @throws Exception
     */
    public function postForm($url, $params = '', $curlOptions = array())
    {
        if (is_array($params)) {
            $params = $this->makePostFields($params);
     ...
     * @param array $curlOptions
     * @return object
     * @throws CallaCharmCurlException
     */
     ...
     * @return CurlService
     */
    public function addCurlOption($name, $value): CurlService
    {
     ...
     * @return CurlService
     */
    public function addCurlOptions($curlOptions = array()): CurlService
    {
     ...
     * @return CurlService
     */
    public function appendRequestHeader($key, $value): self
    {
        if (is_string($key)) {
     ...
     * @return CurlService
     */
    public function appendRequestHeaders(array $headers): self
    {
        foreach ($headers as $name => $value) {
            $this->appendRequestHeader($name, $value);
        }
    ...
    protected function request($url, $curlOptions = array())
    {
        ...
        $response = Response::json([
            'content' => $httpBody
        ], $this->info['http_code'], $this->responseHeaders);
        ...
        if (! empty($this->error)) {
            throw new CallaCharmCurlException($this->error);
        }
    ...
    private function prepareRequest($curl, $curlOptions)
    {
        ...
        curl_setopt($curl, CURLOPT_HTTPHEADER, self::prepareRequestHeadersFrom($this->requestHeaders));
    ...
    private static function prepareRequestHeadersFrom($headers): array
    {
        $processedHeaders = [];
